...
import { AlpanaBorder, FolkFish, Reveal, WaveLine } from "../components/folk";
import { DishCard } from "../components/sections";
import { SectionHead, WhatsAppLink } from "../components/ui-kit";
import { FEATURED_DISHES, MENU } from "../content/menu";
...
export const Route = createFileRoute("/menu")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: MENU_PAGE.seo.description },
      { property: "og:title", content: MENU_PAGE.seo.ogTitle },
      { property: "og:description", content: MENU_PAGE.seo.ogDescription },
  ...
  return (
    <>
      <section className="paper relative overflow-hidden bg-cream px-5 pb-8 pt-10 sm:px-6 lg:pt-16">
        <FolkFish className="float-slow pointer-events-none absolute -right-3 bottom-2 h-10 w-20 text-brick/15" />
      ...
      <section className="bg-cream px-5 pb-16 pt-6 sm:px-6 lg:pb-24">
        <div className="mx-auto max-w-6xl">
          <p className="eyebrow text-brick/70">{category.blurb}</p>
                ...
                <Reveal as="li" key={item} delay={i * 60}>
                  <div className="flex h-full overflow-hidden rounded-3xl border-2 border-brick/12 bg-paint">
                    ...
                    <div className="flex min-w-0 flex-col justify-center p-4">
                      ...
                      {f && <p className="mt-1 text-sm text-muted-foreground">{f.desc}</p>}
                    </div>
                  </div>
                </Reveal>
              );
          ...
          <p className="mt-8 text-sm italic text-muted-foreground">{MENU_PAGE.pricingNote}</p>
          <div className="mt-6">
            <WhatsAppLink message={MENU_PAGE.whatsappMessage}>
      ...
      <section className="bg-cream py-14 lg:py-20">
        <div className="mx-auto max-w-6xl px-5 sm:px-6">
          <SectionHead eyebrow={MENU_PAGE.signature.eyebrow} title={MENU_PAGE.signature.title} />
        </div>
        <div className="rail mt-8 px-5 sm:px-6 lg:mx-auto lg:max-w-6xl">
          {Object.entries(FEATURED_DISHES).map(([name, f], i) => (
            <Reveal key={name} delay={i * 70}>
              <DishCard image={f.image} name={name} desc={f.desc} />
            </Reveal>
          ))}
        </div>
      </section>
